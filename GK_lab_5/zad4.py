...
# Zmienna kontrolująca, która tekstura jest aktualnie aktywna
texture_switch = 0  # 0 - tekstura 1, 1 - tekstura 2

# Wczytanie obu tekstur do pamięci
texture_1 = Image.open("P6_t.tga")
texture_2 = Image.open("N2_t.tga")

# Inicjalizacja tekstur
texture_id_1 = glGenTextures(1)
texture_id_2 = glGenTextures(1)

def load_texture(texture_image, texture_id):
    glBindTexture(GL_TEXTURE_2D, texture_id)
    glTexImage2D(GL_TEXTURE_2D, 0, 3, texture_image.size[0], texture_image.size[1], 0,
...
def startup():
    ...
    load_texture(texture_1, texture_id_1)
    load_texture(texture_2, texture_id_2)

    # Inicjalizacja materiałów i światła (pozostaje bez zmian)
...
def render(time):
    global theta, move_up_down, texture_switch
    ...
    # Podstawowe rysowanie trójkątów z teksturą
    # Kolejne ściany...
    glBegin(GL_TRIANGLES)
    glTexCoord2f(1.0, 0.0)
    glVertex3f(5.0,  -5.0, 0.0)
    ...
    # Zmienna 'texture_switch' kontroluje, która tekstura jest aktywna
    glFlush()
...
def keyboard_key_callback(window, key, scancode, action, mods):
    global x_key_pressed, move_up_down, texture_switch
    ...
    if key == GLFW_KEY_T and action == GLFW_PRESS:  # Przełączanie tekstury
        texture_switch = 1 - texture_switch  # Toggle between textures
