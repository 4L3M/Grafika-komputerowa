...
def compile_shaders():
    vertex_shader_source = """
        ...
        void main(void) {
            // Wykorzystanie gl_InstanceID, aby przesunąć każdą instancję obiektu
            int x = (gl_InstanceID % 10) - 4;  // Obliczanie pozycji instancji w osi X
            int y = (gl_InstanceID / 10) - 4;  // Obliczanie pozycji instancji w osi Y
            mat4 translation_matrix = mat4(1.0);
            translation_matrix[3] = vec4(x, y, 0.0, 1.0); // Translacja instancji
        
            // Zastosowanie translacji, obrotu oraz projekcji
            gl_Position = P_matrix * V_matrix * M_matrix * translation_matrix * vec4(position, 1.0);
...
def startup():
    ...
    vertex_data = numpy.array([
        ...
        -0.25,  0.25,  0.25,  0.0, 1.0, 0.0, 1.0,
        -0.25, -0.25,  0.25,  0.0, 1.0, 0.0, 1.0,
         0.25, -0.25,  0.25,  0.0, 1.0, 0.0, 1.0,
         0.25, -0.25,  0.25,  0.0, 1.0, 0.0, 1.0,
         0.25,  0.25,  0.25,  0.0, 1.0, 0.0, 1.0,
        ...
        -0.25, -0.25, -0.25,  1.0, 1.0, 0.0, 1.0,
         ...
         0.25,  0.25, -0.25,  1.0, 0.0, 1.0, 1.0,
         0.25,  0.25,  0.25,  1.0, 0.0, 1.0, 1.0,
         0.25, -0.25,  0.25,  1.0, 0.0, 1.0, 1.0,
         0.25, -0.25,  0.25,  1.0, 0.0, 1.0, 1.0,
         0.25, -0.25, -0.25,  1.0, 0.0, 1.0, 1.0,
        ...
        -0.25,  0.25, -0.25,  0.0, 1.0, 1.0, 1.0,
        -0.25,  0.25,  0.25,  0.0, 1.0, 1.0, 1.0,
        -0.25, -0.25,  0.25,  0.0, 1.0, 1.0, 1.0,
        -0.25, -0.25,  0.25,  0.0, 1.0, 1.0, 1.0,
        -0.25, -0.25, -0.25,  0.0, 1.0, 1.0, 1.0,
...
def render(time):
    ...
    # Tworzymy planszę 10x10
    # for i in range(10):
    #     for j in range(10):
    #         translation_matrix = glm.translate(glm.mat4(1.0), glm.vec3(i - 4.5, j - 4.5, 0.0))
    #         final_matrix = translation_matrix * M_matrix  # Zastosowanie translacji do obrotu
    #
    #         glUniformMatrix4fv(M_location, 1, GL_FALSE, glm.value_ptr(final_matrix))
    #
...
def main():
    ...
    window = glfwCreateWindow(400, 400, "OpenGL", None, None)
    if not window:
        glfwTerminate()
        sys.exit(-1)
