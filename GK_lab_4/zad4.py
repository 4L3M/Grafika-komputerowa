...
mat_ambient = [0.5, 0.7, 0.8, 1.0]  # Jasno niebieski
mat_diffuse = [0.5, 0.7, 0.8, 1.0]  # Jasno niebieski
mat_specular = [1.0, 1.0, 0.8, 1.0]
mat_shininess = 20.0

light_ambient = [0.1, 0.1, 0.0, 1.0]
light_diffuse = [0.8, 0.2, 8.0, 1.0]
...
n = 20
coordinates_matrix = np.zeros((n + 1, n + 1, 3))
normal_vectors_matrix = np.zeros((n + 1, n + 1, 3))
...
# Function to map normal vectors to colors
...
def fill_coordinates_matrix():
    for i in range(0, n + 1):
        for j in range(0, n + 1):
            u = i / n
            v = j / n
            coordinates_matrix[i][j][0] = (-90 * pow(u, 5) + 225 * pow(u, 4) - 270 * pow(u, 3) + 180 * pow(u, 2) - 45 * u) * cos(
                pi * v)
            coordinates_matrix[i][j][1] = 160 * pow(u, 4) - 320 * pow(u, 3) + 160 * pow(u, 2)
            coordinates_matrix[i][j][2] = (-90 * pow(u, 5) + 225 * pow(u, 4) - 270 * pow(u, 3) + 180 * pow(u, 2) - 45 * u) * sin(
                pi * v)


def fill_normal_vectors_matrix():
    for i in range(0, n + 1):
        for j in range(0, n + 1):
            u = i / n
            v = j / n
...
def draw_egg_surface():
    for i in range(0, n):
        for j in range(0, n):
            ...
            # Second triangle
            glNormal3f(normal_vectors_matrix[i][j+1][0], normal_vectors_matrix[i][j+1][1], normal_vectors_matrix[i][j+1][2])
...
def main():
    ...
    random.seed(1)
    fill_normal_vectors_matrix()
    fill_coordinates_matrix()
